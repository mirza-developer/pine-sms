# Performance Optimization Plan — `PineSms.OrderTrack`
...
- `PineSms.OrderTrack/wwwroot/index.html` —
...
- `PineSms.OrderTrack/Layout/SiteHeader.razor` — header logo currently has
  `loading="lazy"`. That image is the **LCP candidate** above the fold; lazy-
  loading it actively delays LCP.
- `PineSms.OrderTrack/Layout/SiteFooter.razor` — footer logo + social icons
  use `loading="lazy"`. Correct as-is (below the fold).
- `PineSms.OrderTrack/wwwroot/lib/bootstrap/**` — present in `wwwroot` but **not
  referenced** by `index.html` or `app.css`. Still ships with publish output
  unless excluded.
- `PineSms.OrderTrack/wwwroot/css/app.css` — 1223 lines, minified to
  `app.min.css` via `bundleconfig.json` ✅.
- `PineSms.OrderTrack/Pages/Home.razor.cs` — only uses `ToEnglishDigits()` from
  `PineSms.Shared`. The `PineSms.Shared` reference may otherwise be unnecessary
...
**Where:** `PineSms.OrderTrack/PineSms.OrderTrack.csproj`, in the existing
`<PropertyGroup>`.
...
**Where:** `PineSms.OrderTrack/Properties/PublishProfiles/IIS.pubxml`.
...
**Where:** `PineSms.OrderTrack/web.config`, inside `<system.webServer>`.
...
**Where:** `PineSms.OrderTrack/web.config`.
...
**Where:** `PineSms.OrderTrack/PineSms.OrderTrack.csproj`.
...
### A6. Trim the `PineSms.Shared` dependency
...
- **(a) Preferred** — drop the `<ProjectReference Include="..\PineSms.Shared\PineSms.Shared.csproj" />`
  from `PineSms.OrderTrack.csproj` and inline `ToEnglishDigits()` into a local
  `PineSms.OrderTrack/Extensions/StringExtensions.cs`.
...
**Where:** `PineSms.OrderTrack/PineSms.OrderTrack.csproj` — Release-only
property group:
...
**Where:** `PineSms.OrderTrack/Layout/SiteHeader.razor`, line 7.
...
**Where:** `PineSms.OrderTrack/wwwroot/index.html`, in `<head>`.
...
**Where:** `PineSms.OrderTrack/wwwroot/index.html`, in `<head>`, **before** the
stylesheet `<link>`.
...
**Where:** `PineSms.OrderTrack/wwwroot/index.html`.
...
**Where:** `PineSms.OrderTrack/wwwroot/index.html`, the `<div id="app">`
content.
...
7. **A6** — trim `PineSms.Shared` dependency
...
```powershell
dotnet publish .\PineSms.OrderTrack\PineSms.OrderTrack.csproj -c Release -o .\publish-perf
