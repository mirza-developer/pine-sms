...
public class BotUpdateHandler : IBotUpdateHandler
{
    ...
    private async Task HandleFeedbackAsync(long userChatId, string feedbackJson, string username, CancellationToken ct)
    {
        ...
        {
            logger.LogWarning("Feedback JSON missing 'Type' field for user {Username}", username);
            return;
        }
        ...
        {
            logger.LogWarning("Feedback JSON missing or invalid 'TargetChatId' for type: {FeedbackType}, user: {Username}", feedbackType, username);
            return;
        }
        ...
        if (targetChatId == 0)
        {
            logger.LogWarning("Chat ID not configured (0) for feedback type: {FeedbackType}", feedbackType);
    ...
    {
        logger.LogInformation("HandleSatisfactionAsync: Sending message to user {ChatId} and group {TargetChatId}", userChatId, targetChatId);
        ...
        logger.LogInformation("Sending satisfaction log to group {TargetChatId}: {Message}", targetChatId, satisfactionLog);
        await botClient.SendMessageAsync(targetChatId, satisfactionLog, groupCt);
        logger.LogInformation("Successfully sent satisfaction log to group {TargetChatId}", targetChatId);
    }
